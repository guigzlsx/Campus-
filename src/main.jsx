...
function Avatar({ initials = 'EM', small = false }) {
  return <div className={`avatar ${small ? 'avatar-small' : ''}`} aria-label="Profil d’Emma Martin">{initials}</div>
}
    ...
    <div className="app-shell" data-theme="campus-plus" style={{ '--ds-accent': AppTheme.colors.violet }}>
      <aside className="sidebar">
        ...
        <button className={`student-card ${activeView === 'profile' ? 'student-card-active' : ''}`} onClick={() => navigate('profile')}>
          ...
          <div className="student-card-name">Emma Martin</div>
        ...
        <div className="sidebar-bottom">
          ...
          <button className="profile-shortcut" onClick={() => navigate('profile')}><Avatar small /><span>Emma Martin</span><span className="more-dots">•••</span></button>
        </div>
      </aside>
      ...
      <main className="main-panel">
        <header className="topbar">
          ...
          <div className="topbar-actions">
            ...
            <button className="top-profile" onClick={() => navigate('profile')}><Avatar small /><span>Emma</span><Icon name="chevron" size={15} /></button>
          </div>
        </header>
...
function HomeView({ navigate, savedEvents, toggleEvent, showToast }) {
  return <div className="view home-view">
    <section className="welcome-row">
      <div><div className="eyebrow">Jeudi 18 septembre 2026</div><h1>Bonjour Emma <span className="wave">✦</span></h1><p className="welcome-copy">Voici ce qui mérite ton attention aujourd’hui.</p></div>
